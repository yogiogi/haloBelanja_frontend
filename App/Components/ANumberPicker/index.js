...
export default class ANumberPicker extends Component<Props, State> {
  ...
  render() {
    const { maxLength, onChangeText, onBlur, onLeftPress, onRightPress, isCircle,
      containerStyle } = this.props;
    const { value } = this.state;

    return (
      isCircle ?
        <View style={[styles.containerCircle, containerStyle]}>
          <TouchableOpacity
            accessibilityLabel="action_minus_anumberpicker"
            onPress={() => { this._onLeftPress(onLeftPress, value, onChangeText); }}
          >
            <Icon
              name="remove-circle"
              style={value > 0 ? styles.circleIconStyle : styles.inactiveIconStyle}
              type="MaterialIcons"
            />
          </TouchableOpacity>
          <ATextInput.CirclePicker
            value={value}
            maxLength={maxLength}
            accessibilityLabel="input_anumberpicker"
            onChangeText={(text) => { this._onChangeText(text, onChangeText); }}
            onBlur={onBlur}
            textStyle={styles.circleInputStyle}
          />
          <TouchableOpacity
            accessibilityLabel="action_plus_anumberpicker"
