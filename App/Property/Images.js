const images = {
  merchant_list: {
    activeMerchant: require('../Images/active_merchant.png'),
    listMerchant: require('../Images/list_merchant.png'),
    profitMerchant: require('../Images/profit_merchant.png'),
  },
  onlineShop: {
    bukalapak: require('../Images/bukalapak.png'),
    tokopedia: require('../Images/tokopedia.png'),
    shopee: require('../Images/shopee.png'),
  },
  showPassword: require('../Images/eye_visible.png'),
  mailActivation: require('../Images/mail_activation.png'),
  verifiedMerchant: require('../Images/verified_merchant.png'),
  facebook: require('../Images/facebook.png'),
  google: require('../Images/google.png'),
};

export default images;
